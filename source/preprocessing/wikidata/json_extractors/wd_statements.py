...
def __get_typed_extractor(property: Properties):
    if property.underlyingType == UnderlyingTypes.ENTITY:
        return _entityids_value_extractor
    elif property.underlyingType == UnderlyingTypes.STRING:
        return _string_value_extractor
    else:
        raise ValueError(f"Missing value extractor for property {property}")
...
def extract_wd_statement_values(wd_json, property: Properties, *, field: str | None = RootFields.CLAIMS, is_qualifier: bool = False, include_no_value: bool = False):
    ...
    typed_extractor = __get_typed_extractor(property)
