...
def __log_context_func(classes_set, properties_set):
    def log_context_message():
        capture_classes_set = classes_set
        capture_properties_set = properties_set 
...
def __is_wd_entity_property_for_separation(str_entity_id: str) -> bool:
    if wd_entity_types.is_wd_entity_property(str_entity_id):
        return True
    else:
        return False

def __mark_classes_for_separation(wd_classes_ids_set: set, values):
    for value in values:
        wd_classes_ids_set.add(value)
        
def __mark_property_for_separation(wd_properties_ids_set: set, value):
    wd_properties_ids_set.add(value)
        
def __process_wd_entity(wd_entity, wd_classes_ids_set: set, wd_properties_ids_set: set):
    try:
        ...
        if wd_entity_types.is_wd_entity_item_or_property(str_entity_id):
            ...
            elif __is_wd_entity_property_for_separation(str_entity_id):
                __mark_property_for_separation(wd_properties_ids_set, str_entity_id)
            else:
                pass
    except:
...
def identify_classes_properties(bz2_dump_file_path: pathlib.Path) -> set:
    ...
    wd_properties_ids_set = set()
    with (bz2.BZ2File(bz2_dump_file_path) as bz2_input_file):
        for wd_entity in decoding.entities_generator(bz2_input_file, logger, ul.ENTITY_PROGRESS_STEP, __log_context_func(wd_classes_ids_set, wd_properties_ids_set)):
            __process_wd_entity(wd_entity, wd_classes_ids_set, wd_properties_ids_set)
        return wd_classes_ids_set, wd_properties_ids_set
