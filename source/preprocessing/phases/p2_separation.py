...
def __log_context_func(class_counter, property_counter, classes_set, properties_set):
    def log_context_message():
        ...
        properties_set_capture = properties_set
        return f"Separated Classes: {classes_counter_capture.get_count():,} / {len(classes_set_capture):,} Properties: {properties_counter_capture.get_count():,} / {len(properties_set_capture):,}"
    return log_context_message

def __log_sum_progress(class_counter, property_counter, classes_set, properties_set):
    class_count = class_counter.get_count()
    property_count = property_counter.get_count()
    classes_set_count = len(classes_set)
    properties_set_count = len(properties_set)
    logger.info(f"Separated Classes: {class_count:,} Properties: {property_count:,} . The sum is {(class_count + property_count):,} from the set of {classes_set_count + properties_set_count:,}")
...
def __process_wd_entity(wd_entity, classes_output_file, properties_output_file, class_counter, property_counter, wd_classes_ids_set: set, wd_properties_ids_set: set) -> None:
    try:
        str_entity_id = wd_json_fields_ex.extract_wd_id(wd_entity)
        if str_entity_id != None:
            if wd_entity_types.is_wd_entity_property(str_entity_id) and str_entity_id in wd_properties_ids_set:
...
def __separate_to_files(bz2_input_file, classes_output_file, properties_output_file, wd_classes_ids_set: set, wd_properties_ids_set: set, class_counter, property_counter):    
    for wd_entity in decoding.entities_generator(bz2_input_file, logger, ul.ENTITY_PROGRESS_STEP, __log_context_func(class_counter, property_counter)):
        __process_wd_entity(wd_entity, classes_output_file, properties_output_file, class_counter, property_counter, wd_classes_ids_set, wd_properties_ids_set) 
    __log_sum_progress(class_counter, property_counter, wd_classes_ids_set, wd_properties_ids_set)

@timed(logger)
def separate_to_files(bz2_dump_file_path: pathlib.Path, wd_classes_ids_set: set, wd_properties_ids_set: set):
    with (bz2.BZ2File(bz2_dump_file_path) as bz2_input_file,
          ...
          bz2.BZ2File(PROPERTIES_OUTPUT_FILE, "w") as properties_output_file
            ...
            __separate_to_files(bz2_input_file, classes_output_file, properties_output_file, wd_classes_ids_set, wd_properties_ids_set, class_counter, property_counter)
