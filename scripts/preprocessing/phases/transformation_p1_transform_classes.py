...
def __log_progress(i):
    logger.info(__log_progress_message(i))

def __try_log_progress(i):
...
def __process_wd_class(wd_class, classes_output_file):
    ...
    if wd_entity_types.is_wd_entity_item(str_class_id):
        new_class = __transform_wd_class(str_class_id, wd_class)
...
def __transform_classes(bz2_input_file, classes_output_file):
    i = 0
    for binary_line in bz2_input_file:
        ...
        i += 1
        __try_log_progress(i)
    __log_progress(i)
    
def transform_classes(bz2_classes_file_path: pathlib.Path):
    with (bz2.BZ2File(bz2_classes_file_path) as bz2_input_file,
          open(CLASSES_OUTPUT_FILE, "wb") as classes_output_file
            ...
            __transform_classes(bz2_input_file, classes_output_file)
