...
def __get_num_ids(str_ids_arr):
    return wd_fields_trans.transform_wd_str_ids_to_num_ids(str_ids_arr)

def __get_num_id(str_id):
    return wd_fields_trans.transform_wd_str_id_to_num_id(str_id)

def __get_num_ids_map(map):
    new_map = {}
    for str_prop, str_values in map.items():
        new_map[str(__get_num_id(str_prop))] = __get_num_ids(str_values)
    return new_map
...
def __extract_constraint_values_for_statement_allowance(wd_entity_json, constraint):
    ...
    return __get_num_ids_map(constraint_statement_value_map)
...
def __make_map_values_unique_num_ids(map):
    new_map = {}
    for key, values in map.items():
        new_map[key] = __get_num_ids(__get_unique_values(values))
    return new_map
...
def __extract_constraint_values_for_subject_value_constrainsts(wd_entity_json, constraint):
    ...
    return __make_map_values_unique_num_ids(constraint_statement_relation_map)
